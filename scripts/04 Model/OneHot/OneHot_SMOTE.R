...
#SMOTE
SMOTE_y <- read.csv(paste(filepath, "/CSVs/SMOTE_y.csv", sep = ""), stringsAsFactors = T)
data_1h <- one_hot(as.data.table(SMOTE_y))

#Loading x
npz1 <- np$load(paste(filepath, "/NPZs/SMOTE_x_matrix.npz", sep = ""))
...
#Loading x
npz1 <- np$load("SMOTE_x_matrix_fs_f_classif.npz")
x_m1 <- as.data.frame(as.data.frame(npz1$f[["indptr"]])[-1,])
colnames(x_m1)[1] <- "x"
...
#Loading x
npz1 <- np$load("SMOTE_x_matrix_fs_chi2.npz")
x_m1 <- as.data.frame(as.data.frame(npz1$f[["indptr"]])[-1,])
colnames(x_m1)[1] <- "x"
