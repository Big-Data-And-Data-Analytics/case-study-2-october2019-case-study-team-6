...
for tech in balancing_technique:
    ...
    if not use_fs_data:
        ...
        if score_function == "chi2":
            X = sc.sparse.load_npz(filepath + '/NPZs/' + balancing_technique[counter] + '_x_matrix_fs_chi2.npz')
            print(f'"x" feature selected with chi2 loaded, {balancing_technique[counter]}')
            fs = pi.load(open(filepath + '/Models/Feature_' + balancing_technique[counter] + 'fs_chi2.tchq', 'rb'))
            print(balancing_technique[counter])
            X_test_chi2 = fs.transform(X_test)
            x_pred = X_test_chi2
        else:
            X = sc.sparse.load_npz(filepath + '/NPZs/' + balancing_technique[counter] + '_x_matrix_fs_f_classif.npz')
    ...
    model = SGDClassifier(n_jobs=6, loss='hinge', penalty='l2', alpha=iterations, random_state=seed, max_iter=iterations, tol=None)
    ...
    if not use_fs_data:
        if metrics.accuracy_score(y_pred, y_test) > acc_thresh:
            filename = filepath + "/Models/" + modelName + "_" + balancing_technique[counter] + "_" + str(acc) + "_" + str(iterations) + ".model"
            pi.dump(model, open(filename, 'wb'))
        else:
            print("Model accuracy below " + acc_thresh + ", model not saved")
    else:
        if score_function == "chi2":
            filename = filepath + "/Models/" + modelName +  "_" + balancing_technique[counter] + "_" + str(acc) + "_" + str(iterations) + "_fs_chi2.model"
            pi.dump(model, open(filename, 'wb'))
        else:
            filename = filepath + "/Models/" + modelName + "_" + balancing_technique[counter] + "_" + str(acc) + "_" + str(iterations) + "_fs_f-classif.model"
            pi.dump(model, open(filename, 'wb'))
