...
for tech in balancing_technique:
    ...
    if use_fs_data == False:
        X = sc.sparse.load_npz(
            filepath + '/NPZs/' + balancing_technique[counter] + '_x_matrix.npz')
    ...
    #### RANDOM FOREST ####
    model = RandomForestClassifier(n_jobs=6, n_estimators=number_of_trees, random_state=seed)
    ...
    disp.ax_.set_title(modelName + " Normalized confusion matrix " + score_function + " Acc: " + str(acc))
    ...
    if not use_fs_data:
        if metrics.accuracy_score(y_pred, y_test) > acc_thresh:
            filename = filepath + "/Models/" + modelName + "_" + balancing_technique[counter] + "_" + str(acc) + "_" + str(number_of_trees) + ".model"
            pi.dump(model, open(filename, 'wb'))
        else:
            print("Model accuracy below " + acc_thresh + ", model not saved")
    else:
        if score_function == "chi2":
            filename = filepath + "/Models/" + modelName +  "_" + balancing_technique[counter] + "_" + str(acc) + "_" + str(number_of_trees) + "_fs_chi2.model"
            pi.dump(model, open(filename, 'wb'))
        else:
            filename = filepath + "/Models/" + modelName + "_" + balancing_technique[counter] + "_" + str(acc) + "_" + str(number_of_trees) + "_fs_f-classif.model"
            pi.dump(model, open(filename, 'wb'))
