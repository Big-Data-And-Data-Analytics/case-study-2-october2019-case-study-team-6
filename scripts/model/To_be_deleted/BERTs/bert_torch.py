...
comment = ["[CLS] " + comment + " [SEP]" for comment in df.onlyTextMotive.values] # Start and end needed for BERT working properly
...
tokenized_texts = [tokenizer.tokenize(sent) for sent in df.onlyTextMotive.values]
...
train_inputs, validation_inputs, train_labels, validation_labels = train_test_split(input_ids, labels, random_state=69, test_size=0.1)
train_masks, validation_masks, _, _ = train_test_split(attention_masks, input_ids, random_state=69, test_size=0.1)
